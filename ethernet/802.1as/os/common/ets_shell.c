...
static void ETS_SetTime(char* aArgString, uint32_t aArgStringLen)
{
    ...
    {
        const ETS_TimeType globalTime = {seconds, nanoseconds};
...
/** @brief Display gPTP Status
    ...
    Set interface magic
    Get interface status using ETS_GetPortStatus()
    Set global magic
...
static void ETS_Status(char* aArgString, uint32_t aArgStringLen)
{
    ...
    ETS_PortStatusType       portStatus;
    ETS_DomainStatusType     domainStatus;
    ETS_DomainPortStatusType domainPortStatus;
    ...
    BCM_MemSet(&domainStatus, 0U, sizeof(domainStatus));
    /* First parse the seconds and nanoseconds fields */
    CONSOLE_SplitLine(aArgString, aArgStringLen, &pingRemains, &pingRemainingLength);

    port = (uint32_t)ETS_Configure_GetNextNumber(pingRemains, pingRemainingLength,
                                       &pongRemains, &pongRemainingLength);

    portStatus.magicId = ETS_PORT_STATUS_MAGIC_ID;
    rc = ETS_GetPortStatus(port, &portStatus);
    if (rc != BCM_ERR_OK) {
        ULOG_ERR("ETS_GetPortStatus for %lu failed %d\n", port, rc);
        goto done;
    }
    ...
    domainPortStatus.magicId = ETS_DOMAIN_PORT_STATUS_MAGIC_ID;
    rc = ETS_GetDomainPortStatus(0, port, &domainPortStatus);
    if (rc != BCM_ERR_OK) {
        ULOG_ERR("ETS_GetDomainPortStatus for %lu failed %d\n", port, rc);
        goto done;
    }
    domainStatus.magicId = ETS_DOMAIN_STATUS_MAGIC_ID;
    rc = ETS_GetDomainStatus(0, &domainStatus);
    if (rc != BCM_ERR_OK) {
        ULOG_ERR("ETS_GetDomainStatus failed %d\n", rc);
        goto done;
    }
    ...
    ULOG_INFO("Active Domain Number    = %u\n", globalStatus.activeDomainNum);
    ...
    ULOG_INFO("------------ Domain 0 Status -----------\n");
    ULOG_INFO("Clock State             = ");
    switch (domainStatus.clockState) {
    ...
    ULOG_INFO("In Rate Ratio           = 0x%x\n", domainStatus.rateRatioIn);
    ULOG_INFO("GM Rate Ratio           = 0x%x\n", domainStatus.gmRateRatio);

    if (0xFFFFUL == domainStatus.slavePort) {
        ULOG_INFO("Slave Port              = None\n");
    } else {
        ULOG_INFO("Slave Port              = %lu\n", domainStatus.slavePort);
    }
    ULOG_INFO("Signaling Tx SeqNum     = %u\n", domainStatus.signalingTxSeqId);
    ULOG_INFO("Requested Sync Interval = %d\n", domainStatus.reqSyncLogInterval);
    ULOG_INFO("Signaling Timer Started = %lu\n", domainStatus.isSignalingTimerStarted);
    ULOG_INFO("Is Sync Received        = %lu\n", domainStatus.isSyncReceived);
    {
        int64_t correctionUS = domainStatus.rxFollowupCorrection/1000LL;
        ULOG_INFO("Rx Correction           = %ldus\n", (int32_t)correctionUS);
    }
    ULOG_INFO("Rx POT                  = %lus : %luns\n",
            (uint32_t)domainStatus.rxPOT.seconds, domainStatus.rxPOT.nanoseconds);
    ULOG_INFO("------------ Port %lu Status -----------\n", port);
    ULOG_INFO("Peer Delay              = %luns\n", portStatus.nbrPropDelay);
    ULOG_INFO("Peer Rate Ratio         = 0x%x\n", portStatus.nbrRateRatio);
    ULOG_INFO("Measuring Delay         = %lu\n", portStatus.isMeasuringPdelay);
    ULOG_INFO("Config Saved            = %lu\n", portStatus.isAVnuPdelayConfigSaved);
    ULOG_INFO("Pdelay Interval         = %d\n", portStatus.currentLogPdelayIntvl);
    ULOG_INFO("Partner Clock Id        = %x:%x:%x:%x:%x:%x:%x:%x\n",
            portStatus.partnerId.clockID.id[0],
            portStatus.partnerId.clockID.id[1],
            portStatus.partnerId.clockID.id[2],
            portStatus.partnerId.clockID.id[3],
            portStatus.partnerId.clockID.id[4],
            portStatus.partnerId.clockID.id[5],
            portStatus.partnerId.clockID.id[6],
            portStatus.partnerId.clockID.id[7]);
    ULOG_INFO("Partner Port            = %u\n", portStatus.partnerId.port);
    ULOG_INFO("Pdelay Tx SeqNum        = %u\n", portStatus.pDelayTxSeqId);
    ULOG_INFO("Lost responses          = %lu\n", portStatus.numPdelayRespLost);
    ULOG_INFO("pDelay Tx State         = %lu\n", portStatus.pDelayReqState);
    ULOG_INFO("pDelay Rx State         = %lu\n", portStatus.pDelayRespState);
    ULOG_INFO("Pdelay T1               = %lus : %luns\n",
            (uint32_t)portStatus.pDelayT1.seconds, portStatus.pDelayT1.nanoseconds);
    ULOG_INFO("Pdelay T2               = %lus : %luns\n",
            (uint32_t)portStatus.pDelayT2.seconds, portStatus.pDelayT2.nanoseconds);
    ULOG_INFO("Pdelay T3               = %lus : %luns\n",
            (uint32_t)portStatus.pDelayT3.seconds, portStatus.pDelayT3.nanoseconds);
    ULOG_INFO("Pdelay T4               = %lus : %luns\n",
            (uint32_t)portStatus.pDelayT4.seconds, portStatus.pDelayT4.nanoseconds);
    ULOG_INFO("------------ Port %lu Domain 0 Status -----------\n", port);
    ULOG_INFO("Sync Interval           = %d\n", domainPortStatus.currentLogSyncIntvl);
    ULOG_INFO("Sync Rx Timeout         = %luus\n",
            (uint32_t)domainPortStatus.syncReceiptTimeoutIntvl);
    ULOG_INFO("Sync Info Avail         = %lu\n", domainPortStatus.syncInfoAvailable);
    ULOG_INFO("Sync SeqNum             = %u\n", domainPortStatus.syncSeqNum);
    ULOG_INFO("Tx Correction           = %luus\n",
            (uint32_t)(domainPortStatus.txFollowupCorrection/1000ULL));
    ULOG_INFO("Sync State              = %lu\n", domainPortStatus.syncState);
    ULOG_INFO("Sync Timestamp          = %lus : %luns\n",
            (uint32_t)domainPortStatus.syncTimestamp.seconds,
            domainPortStatus.syncTimestamp.nanoseconds);
