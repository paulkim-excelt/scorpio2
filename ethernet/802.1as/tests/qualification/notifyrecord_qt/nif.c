...
int32_t NIF_GetCtrlIdx(ETHSWT_VLANIDType aVlanID,
                       uint16_t aCompID,
                       uint8_t aInstID,
...
void SendSyncPacket(BCM_TimeType *systemTime, ETHSWT_MgmtInfoType *mgmtInfo)
{
    ETS_TimestampType   etsTimeStamp;
    ETHER_TimestampType timestamp;
    ETHER_TimestampQualType timestampQual = ETHER_TIMESTAMPQUAL_VALID;
    etsTimeStamp.seconds = systemTime->s;
    etsTimeStamp.nanoseconds = systemTime->ns;
    ...
    timestamp.seconds = etsTimeStamp.seconds + 5ULL;
    timestamp.nanoseconds = etsTimeStamp.nanoseconds;
    ETS_RxPktTSIndication(0ULL, sync_pkt, mgmtInfo->portIdx, &timestamp, &timestampQual);
}

void SendFollowUpPacket(BCM_TimeType *systemTime, ETHSWT_MgmtInfoType *mgmtInfo)
{
    ...
    nanoseconds = CPU_NativeToBE32(systemTime->ns);
            ...
            if (buffers[i].state == USED) {
                ...
                if(((buffers[i].buf[0U] & ETS_MSG_TYPE_MASK) == ETS_MSG_SYNC) ||
                    ...
                    ((buffers[i].buf[0U] & ETS_MSG_TYPE_MASK) == ETS_MSG_PDELAY_RESP)) {
                        ETS_TxPktTSIndication(buffers[i].ctrlIdx, i, 0UL, &timestamp, &timestampQual);
                }
...
{
    SetEvent(TestETHERNET_Task, SystemEvent0);
}
