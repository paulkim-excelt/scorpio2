...
int32_t ValidateRecord(ETS_RecordsListType* const aRecord)
{
            ...
            for (i = 0UL; i < validRecords; ++i) {
                    ...
                    retVal = MsgToIndex((flag & ETS_RECORDDEFS_IS_TX_MASK) >> ETS_RECORDDEFS_IS_TX_SHIFT,
                    ...
                    if(0UL == msgIdx) {
                         if(syncSeqNum != ((aRecord->records[i].flag & ETS_RECORDDEFS_SEQ_NUM_MASK) >> ETS_RECORDDEFS_SEQ_NUM_SHIFT)) {
                             retVal = BCM_ERR_DATA_INTEG;
                             ULOG_ERR("***ERROR***\n");
                             ULOG_ERR("Port No::  : %lu\n", idx);
                             ULOG_ERR("SYNC SeqNo Received : %lu\n", (aRecord->records[i].flag & ETS_RECORDDEFS_SEQ_NUM_MASK) >> ETS_RECORDDEFS_SEQ_NUM_SHIFT);
...
TASK(TestTask)
{
    ...
    *cmdResp.payloadPtr.isNotifyingRecord = TRUE;
    ...
    BCM_ActivateTask(TestETHERNET_Task);
    (void)BCM_WaitEvent(SystemEvent0);
    (void)BCM_ClearEvent(SystemEvent0);
