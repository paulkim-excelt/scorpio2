...
TASK(ETS_ProtocolTask)
{
    ...
    while (tick_count < 128UL) {
        ...
        if (1UL == slaveMode) {
            ...
            if (0UL == tick_count % 2UL) {
                ...
                seconds = timeStamp.seconds + 100ULL;
                nanoseconds = CPU_NativeToBE32(timeStamp.nanoseconds);
    ...
    if (ETS_CLOCKMODE_GM == ets_config.clockMode) {
        timeStamp.seconds = 100000ULL;
        timeStamp.nanoseconds = 0UL;
        ret = ETS_SetGlobalTime(&timeStamp);
        CHECK_ERR(ret,__LINE__);
    }

    ret = ETS_GetCurrentTime(&timeStamp, &timestampState);
    CHECK_ERR(ret,__LINE__);
    ...
    if (1UL == slaveMode) {
        ...
        if(timeStamp.seconds < 100000ULL) {
            ULOG_ERR("Error: Time: %lus \n", timeStamp.seconds);
            etslib_qt_result = BCM_ERR_UNKNOWN;
            return;
        }
