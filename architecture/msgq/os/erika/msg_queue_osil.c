...
*/
#define BRCM_SWDSGN_MSGQ_SYSCMD_PROC         (0xE101U)    /**< @brief #MSGQ_SysCmdReq  */
#define BRCM_SWDSGN_MSGQ_CMD_HANDLER_PROC    (0xE102U)    /**< @brief #MSGQ_CmdHandler */
#define BRCM_SWDSGN_MSGQ_IOTYPE_PROC         (0xE103U)    /**< @brief #MSGQ_SVCIOType  */
/** @} */
                        ...
                        MSGQ_IOType * const aMsgQIO)
{
    ...
    if (NULL != aMsgQIO) {
        ...
        if ((BCM_ERR_OK == retVal) && (MSGQ_INVALID_CLIENT != aMsgQIO->signalID)
...
void MSGQ_CmdHandler(uint32_t aMagic, uint32_t aCmd, uint8_t * aMsgQSysIO)
{
    ...
    if ((NULL != aMsgQSysIO) && (aMagic == SVC_MAGIC_MSG_ID)) {
        ...
        MSGQ_IntCmdHandler(aCmd, (int32_t)TaskID, msgq.io);
    }
}
