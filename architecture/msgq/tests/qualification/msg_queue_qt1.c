...
/** @brief BRCM_SWQTSEQ_MSGQ_EN_NOTIF_1
 *
 * This file implements BRCM_SWQTSEQ_MSGQ_EN_NOTIF_1, which tests the message queue with
...
TASK(MsgQQT1_ServerTask)
{
    ...
    do {
        ...
        if (0UL != (mask & MsgQQT1_TriggerServer)) {
            ...
            if (ret == BCM_ERR_OK) {
                REPORT_RESULT(BRCM_SWQTST_MSGQ_GET_MSG_IDX_POS, ret == BCM_ERR_OK)
                ...
                REPORT_RESULT(BRCM_SWQTST_MSGQ_COMPLETE_MSG_IDX_POS, ret == BCM_ERR_OK)
                if (BCM_ERR_OK != ret) {
                    whileCondition = 0UL;
                    /*break;
                ...
                REPORT_RESULT(BRCM_SWQTST_MSGQ_GET_MSG_IDX_POS, ret == BCM_ERR_BUSY)
                whileCondition = 0UL;
                /*break;
                * Since while condition is now false, so break statement is not required*/
            } else {
...
TASK(MsgQQT1_TaskClient1)
{
    ...
    do {
        ...
        REPORT_RESULT(BRCM_SWQTST_MSGQ_SEND_MSG_POS, ret == BCM_ERR_OK)
        if (ret == BCM_ERR_OK) {
            ...
            REPORT_RESULT(BRCM_SWQTST_MSGQ_RECV_MSG_POS, ret == BCM_ERR_OK)
        }
        if (ret != BCM_ERR_OK) {
            break;
        }
...
TASK(MsgQQT1_TaskClient2)
{
    ...
    do {
        ...
        REPORT_RESULT(BRCM_SWQTST_MSGQ_SEND_MSG_POS_2, ret == BCM_ERR_OK)
        if (ret == BCM_ERR_OK) {
            /* Wait for message completion signal from server/queue */
            ...
            /* Receive response from server/queue */
            ret = MSGQ_RecvMsg(&MsgQQT1_msgQ, pHdr, &msgResp);
            REPORT_RESULT(BRCM_SWQTST_MSGQ_RECV_MSG_POS_2, ret == BCM_ERR_OK)
        }
        if (ret != BCM_ERR_OK) {
            break;
        }
...
int32_t MsgQQT1_GetResult(void)
{
    ...
    if (BCM_AT_EXECUTING == ret) {
        ...
        REPORT_RESULT(BRCM_SWQTSEQ_MSGQ_EN_NOTIF_1, ret)
    }
    return ret;
}
