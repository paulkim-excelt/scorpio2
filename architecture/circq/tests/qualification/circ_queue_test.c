...
void CIRCQ_QualReport (uint32_t aConsoleID,
                       uint16_t aCompId,
                       uint32_t aTestSeq,
                       uint32_t aTestCase,
                       uint32_t aStatus)
...
int32_t CircQQT_Start(void)
{
    ...
    itemsPeeked = CIRCQ_Peek(&CIRC_Q, (char *)bufpeekArray, 1UL);
    if (0UL == itemsPeeked) {
    ...
    if (0UL == CIRCQ_Pop(&CIRC_Q, (char *)bufpopArray, 1UL)) {
    ...
    retVal = CIRCQ_Push(&CIRC_Q, (const char *)bufpushArrayOne, 1UL);
    ...
    if (1UL == CIRCQ_Pop(&CIRC_Q, (char *)bufpopArray, 1UL)) {
    ...
    itemsPeeked = CIRCQ_Peek(&CIRC_Q, (char *)bufpeekArray, 2UL);
    if (2UL == itemsPeeked) {
    ...
    retVal = CIRCQ_Push(&CIRC_Q,(const char *)bufpushArrayAll, 15UL);
    if (BCM_ERR_OK == retVal) {
    ...
    itemsPeeked = CIRCQ_Peek(&CIRC_Q, (char *)bufpeekArray, 15UL);
    if (itemsPeeked == 15UL) {
    ...
    itemsPeeked = CIRCQ_Peek(&CIRC_Q, (char *)bufpeekArray, 14UL);
    if (itemsPeeked == 14UL) {
    ...
    if (15UL == CIRCQ_Pop(&CIRC_Q, (char *)bufpopArray, 15UL)) {
