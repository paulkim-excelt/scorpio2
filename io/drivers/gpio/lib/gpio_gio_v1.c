...
static uint32_t PINMUX_IsInitialized COMP_SECTION(".bss.drivers");
                      ...
                      const PINMUX_PinModeCfgType* const aConfigPtr)
{
    ...
    if (aPin < GIO_MAX_CHANNELS) {
        ...
        if (PINMUX_PIN_MODE_GPIO == mode) {
            ...
            GPIO_Regs[port]->pwm_sel &= (~(GIO_AUXSEL_AUX << pin_offset));
        } else {
...
void GPIO_DrvIRQHandler(GPIO_PortType aPort)
{
    ...
    if (GIO_MAX_PORTS > aPort) {
        ...
        while (0UL != irq_status) {
            if (1UL == (irq_status & 0x1UL)) {
                if (NULL != GPIO_IntrCbTbl[channelId]) {
                    ...
                    values[3] = channelId;
                    BCM_ReportError(BCM_GIO_ID, 0U, BRCM_SWARCH_GPIO_DRV_IRQHANDLER_PROC, BCM_ERR_UNKNOWN,
                                    4UL, values);
                }
            }
            channelId++;
        ...
        values[3] = aPort;
        BCM_ReportError(BCM_GIO_ID, 0U, BRCM_SWARCH_GPIO_DRV_IRQHANDLER_PROC, BCM_ERR_INVAL_PARAMS,
                        4UL, values);
    }
}
 ...
 * @return       Error type
 */
static int32_t GIO_EnableInterrupt(GPIO_ChannelType aChannelId)
{
    GPIO_PortType port;
    uint32_t pinMask;
 ...
 * @return       Error type
 */
static int32_t GIO_DisableInterrupt(GPIO_ChannelType aChannelId)
{
    GPIO_PortType port;
    uint32_t pinMask;
    int32_t ret = BCM_ERR_OK;
 ...
 */
static GPIO_LevelType GIO_ReadChannel(GPIO_ChannelType aChannelId)
{
 ...
 * @return       void
 */
static void GIO_WriteChannel(GPIO_ChannelType aChannelId, GPIO_LevelType aLevel)
{
 ...
 */
static GPIO_PortLevelType GIO_ReadPort(GPIO_PortType aPortId)
{
 ...
 * @return       void
 */
static void GIO_WritePort(GPIO_PortType aPortId, GPIO_PortLevelType aLevel)
{
    if (aPortId < GIO_MAX_PORTS) {
 ...
 */
static GPIO_PortLevelType GIO_ReadChannelGroup(const GPIO_ChannelGroupType* aChannelGroupIdPtr)
{
    GPIO_PortLevelType lvl;
 ...
 * @return       void
 */
static void GIO_WriteChannelGroup(const GPIO_ChannelGroupType* aChannelGroupIdPtr,
 ...
 */
static GPIO_LevelType GIO_FlipChannel(GPIO_ChannelType aChannelId)
{
 ...
 * @return       void
 */
static void GIO_SetPinValueDirection(PINMUX_PinType aPin, PINMUX_PinDirectionType aDirection)
{
    PINMUX_PinType p;
 ...
 * @return       void
 */
static void GIO_RefreshPortDirection(void)
{
    PINMUX_PinType pin;
 ...
 * @return       void
 */
static void GIO_SetPinMode(PINMUX_PinType aPin, PINMUX_PinModeType aMode)
{
    PINMUX_PinType p;
    uint32_t mode;
...
/**
    ...
    for Type conversion
*/
typedef union uPINMUX_SVCIOType {
    uint8_t *data;
    PINMUX_IOType *io;
} PINMUX_SVCIOType;
