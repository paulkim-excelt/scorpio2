...
class CFG_DataType:
    ...
    def __contruct__(self, aSchema):
        ...
        if 'type' in self.properties.keys():
            self.typeName = self.properties['type']
    ...
    def getParentType(self, aElementName, aXSDDataTypes):
        for child in self.typeObj:
            for gChild in child.children:
                if gChild.tagName == 'choice':
                    allElements = True
                    for each in gChild.children:
                        if each.tagName != 'element':
                            allElements = False
                            break
                    return True, allElements
                else:
                    return False, False
                    ...
                    for gChild in child.children:
                        if gChild.getName() is not None:
                            if gChild in aChildList:
                                found = True
                                maxOccurs = 1
                                if 'maxOccurs' in gChild.properties:
                                    if gChild.properties['maxOccurs'] != 'unbounded':
                                        maxOccurs = int(gChild.properties['maxOccurs'])
                                    else:
                                        maxOccurs = float('inf')

                                if maxOccurs > aChildList.count(gChild):
                                    childList.append(gChild)
                                break
                        else:
...
class CFG_SchemaParser:
    ...
    def isChoice(self, aPath, aElementName):
        curNode = self.root.getCurNode(aPath)
        if curNode is None:
            return None
        (isChoice, allElements) = curNode.getParentType(aElementName, self.xsdDataTypes())
        if isChoice == True and allElements == True:
            types = self.getTypes(aPath + [aElementName])
            isXSDType = True
            for each in types:
                if each not in self.xsdDataTypes():
                    isXSDType = False
                    allElements = False
                    break
        return isChoice, allElements
    ...
    def getPosition(self, aPath, aCurChild, aSelection):
        ...
        if selection is not None:
            selCount = aCurChild.count(aSelection)
            return curNode.getActivePosition(childList, selection, children) + selCount
        else:
            return None
