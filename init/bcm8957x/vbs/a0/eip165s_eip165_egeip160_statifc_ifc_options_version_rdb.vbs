...
'' @REGISTER : EIP165S_EIP165_EGEIP160_STATIFC_IFC_OPTIONS_VERSION_ifc_count_options2
'' @API      : data
''           : read
''           : write
''           : get_
...
''                                                             write_DW
''---------------------------------------------------------------------------------
Class REGISTER_EIP165S_EIP165_EGEIP160_STATIFC_IFC_OPTIONS_VERSION_ifc_count_options2
    Private write_NUM_IFC_value
    Private read_NUM_IFC_value
    Private flag_NUM_IFC
    ...
    Property Get get_NUM_IFC
        get_NUM_IFC = read_NUM_IFC_value
    End Property

    Property Let set_NUM_IFC(aData)
        write_NUM_IFC_value = aData
        flag_NUM_IFC        = &H1
    End Property

    Property Get read_NUM_IFC
        read
        read_NUM_IFC = read_NUM_IFC_value
    End Property

    Property Let write_NUM_IFC(aData)
        set_NUM_IFC = aData
        write
    End Property
    ...
    Sub read
        ...
        if data_low > LONG_MAX then
            if NUM_IFC_mask = mask then
                read_NUM_IFC_value = data_low
            else
                read_NUM_IFC_value = (data_low - H8000_0000) and NUM_IFC_mask
            end If
        else
            read_NUM_IFC_value = data_low and NUM_IFC_mask
        end If
    ...
    Sub write
        If flag_NUM_IFC = &H0 or flag_DW = &H0 Then read
        If flag_NUM_IFC = &H0 Then write_NUM_IFC_value = get_NUM_IFC
        If flag_DW = &H0 Then write_DW_value = get_DW

        regValue = leftShift((write_NUM_IFC_value and &H3fff), 0) + leftShift((write_DW_value and &Hff), 16)
...
'' @REGISTER : EIP165S_EIP165_EGEIP160_STATIFC_IFC_OPTIONS_VERSION_ifc_count_options
'' @API      : data
''           : read
''           : write
''           : get_
...
''                                                             write_NGOC
''---------------------------------------------------------------------------------
Class REGISTER_EIP165S_EIP165_EGEIP160_STATIFC_IFC_OPTIONS_VERSION_ifc_count_options
    Private write_NIPC_value
    Private read_NIPC_value
    Private flag_NIPC
    Private write_NIOC_value
    Private read_NIOC_value
    Private flag_NIOC
    ...
    Property Get get_NIPC
        get_NIPC = read_NIPC_value
    End Property

    Property Let set_NIPC(aData)
        write_NIPC_value = aData
        flag_NIPC        = &H1
    End Property

    Property Get read_NIPC
        read
        read_NIPC = read_NIPC_value
    End Property

    Property Let write_NIPC(aData)
        set_NIPC = aData
        write
    End Property

    Property Get get_NIOC
        get_NIOC = read_NIOC_value
    End Property

    Property Let set_NIOC(aData)
        write_NIOC_value = aData
        flag_NIOC        = &H1
    End Property

    Property Get read_NIOC
        read
        read_NIOC = read_NIOC_value
    End Property

    Property Let write_NIOC(aData)
        set_NIOC = aData
        write
    End Property
    ...
    Sub read
        ...
        NIPC_mask = &H1f
        if data_low > LONG_MAX then
            if NIPC_mask = mask then
                read_NIPC_value = data_low
            else
                read_NIPC_value = (data_low - H8000_0000) and NIPC_mask
            end If
        else
            read_NIPC_value = data_low and NIPC_mask
        end If
        read_NIOC_value = rightShift(data_low, 5) and &H7
    ...
    Sub write
        If flag_NIPC = &H0 or flag_NIOC = &H0 or flag_NGPC = &H0 or flag_NGOC = &H0 Then read
        If flag_NIPC = &H0 Then write_NIPC_value = get_NIPC
        If flag_NIOC = &H0 Then write_NIOC_value = get_NIOC
        If flag_NGPC = &H0 Then write_NGPC_value = get_NGPC
        If flag_NGOC = &H0 Then write_NGOC_value = get_NGOC

        regValue = leftShift((write_NIPC_value and &H1f), 0) + leftShift((write_NIOC_value and &H7), 5) + leftShift((write_NGPC_value and &H1f), 8) + leftShift((write_NGOC_value and &H7), 13)
        RegWrite regWidth, address, regValue
        clear_all_fields
    End Sub
...
'' @REGISTER : EIP165S_EIP165_EGEIP160_STATIFC_IFC_OPTIONS_VERSION_ifc_count_version
'' @API      : data
''           : read
''           : write
''           : get_
...
''                                                             write_MAJO_VERS
''---------------------------------------------------------------------------------
Class REGISTER_EIP165S_EIP165_EGEIP160_STATIFC_IFC_OPTIONS_VERSION_ifc_count_version
...
Class EIP165S_EIP165_EGEIP160_STATIFC_IFC_OPTIONS_VERSION_INSTANCE

    Public ifc_count_options2
    Public ifc_count_options
    Public ifc_count_version


    Public default function Init(aBaseAddr)
        Set ifc_count_options2 = (New REGISTER_EIP165S_EIP165_EGEIP160_STATIFC_IFC_OPTIONS_VERSION_ifc_count_options2)(aBaseAddr, 32)
        Set ifc_count_options = (New REGISTER_EIP165S_EIP165_EGEIP160_STATIFC_IFC_OPTIONS_VERSION_ifc_count_options)(aBaseAddr, 32)
        Set ifc_count_version = (New REGISTER_EIP165S_EIP165_EGEIP160_STATIFC_IFC_OPTIONS_VERSION_ifc_count_version)(aBaseAddr, 32)
...
Set EIP165S_EIP165_EGEIP160_STATIFC_IFC_OPTIONS_VERSION = CreateObject("System.Collections.ArrayList")
EIP165S_EIP165_EGEIP160_STATIFC_IFC_OPTIONS_VERSION.Add ((New EIP165S_EIP165_EGEIP160_STATIFC_IFC_OPTIONS_VERSION_INSTANCE)(&H4d010000))
EIP165S_EIP165_EGEIP160_STATIFC_IFC_OPTIONS_VERSION.Add ((New EIP165S_EIP165_EGEIP160_STATIFC_IFC_OPTIONS_VERSION_INSTANCE)(&H4d110000))
EIP165S_EIP165_EGEIP160_STATIFC_IFC_OPTIONS_VERSION.Add ((New EIP165S_EIP165_EGEIP160_STATIFC_IFC_OPTIONS_VERSION_INSTANCE)(&H4d210000))
EIP165S_EIP165_EGEIP160_STATIFC_IFC_OPTIONS_VERSION.Add ((New EIP165S_EIP165_EGEIP160_STATIFC_IFC_OPTIONS_VERSION_INSTANCE)(&H4d310000))
EIP165S_EIP165_EGEIP160_STATIFC_IFC_OPTIONS_VERSION.Add ((New EIP165S_EIP165_EGEIP160_STATIFC_IFC_OPTIONS_VERSION_INSTANCE)(&H4d410000))
EIP165S_EIP165_EGEIP160_STATIFC_IFC_OPTIONS_VERSION.Add ((New EIP165S_EIP165_EGEIP160_STATIFC_IFC_OPTIONS_VERSION_INSTANCE)(&H4d510000))
