...
*/
#define BRCM_SWARCH_SYS_RPC_PROCESS_CMDS_PROC    (0x9101U)    /**< @brief #SYS_RpcProcessCmds */
/** @} */
...
*/
void  SYS_RpcProcessCmds(void);

/** @brief Function to process commands in System Task
...
*/
void  SYSTEM_ProcessRpcCmds(BCM_SubStateType *const aCmdState);
...
/** @brief Get Pending request
    ...
    @param[out]      aID        Command/Request ID that is got
    @param[out]      aRequest   Request valid only when return value is BCM_ERR_OK

    Return values are documented in reverse-chronological order
    @retval     #BCM_ERR_OK             Command status retrieved successfully
    @retval     #BCM_ERR_NOT_FOUND      Command not found
    @retval     #BCM_ERR_INVAL_PARAMS   aRequest/aID is NULL
...
*/
int32_t SYS_GetRequest(SYS_RpcCmdIdType *aID, SYS_HandleType *aRequest);
