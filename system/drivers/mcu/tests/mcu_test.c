...
{
    MCU_TEST_SVC_IOType *mcuTestSvc;
    mcuTestSvc = (MCU_TEST_SVC_IOType *)aSysIO;
    mcuTestSvc->retVal = BCM_ERR_OK;

    switch (aCmd) {
        case MCU_TEST_SVC_CMD_MCU_INIT:
            MCU_Init(mcuTestSvc->config);
            break;
#ifdef ENABLE_MCU_PADCFG_TEST
        case MCU_TEST_SVC_CMD_PADCFG_TEST:
            MCU_TestPadCfg();
            break;
#endif
        default:
            mcuTestSvc->retVal = BCM_ERR_INVAL_PARAMS;
            break;
    }
}
