...
#define BRCM_SWDSGN_UART_DRV_GET_STATS_PROC        (0xB105U)    /**< @brief #UART_DrvGetStats    */
...
#define BRCM_SWDSGN_UART_SVCIO_TYPE                (0xB109U)    /**< @brief #UART_SVCIOType      */
#define BRCM_SWDSGN_UART_SYS_CMD_HANDLER_PROC      (0xB10AU)    /**< @brief #UART_SysCmdHandler  */
...
static void UART_DrvRxHandler(UART_HwIDType aId)
{
    ...
    while ((flag & UART_UARTFR_RXFE_MASK) != UART_UARTFR_RXFE_MASK) {
        ...
        if ((dataReg & UART_ERR_MASK) != 0UL) {
            ...
            data = ((uint8_t)dataReg) & ((uint8_t)UART_UARTDR_DATA_MASK);
...
/**
    ...
    @trace #BRCM_SWREQ_UART_COMMUNICATION
*/
static int32_t UART_DrvSend(UART_HwIDType aId, const uint8_t* aData, uint32_t aSize)
{
 ...
 *  return      void
    ...
    @trace #BRCM_SWREQ_UART_COMMUNICATION
 */
static int32_t UART_DrvSendBreak(UART_HwIDType aId)
{
 ...
 *  return      void
    ...
    @trace #BRCM_SWREQ_UART_COMMUNICATION
 */
static int32_t UART_DrvClrBreak(UART_HwIDType aId)
{
    int32_t ret = BCM_ERR_INVAL_PARAMS;
    if (aId < UART_MAX_HW_ID) {
        UART_Regs[aId]->uartlcr &= ~UART_UARTLCR_BRK_MASK;
...
/**
    ...
    @trace #BRCM_SWREQ_UART_INIT
*/
static int32_t UART_DrvGetStats(UART_HwIDType aId, UART_StatsType *const aStats)
{
...
/**
    ...
    @trace #BRCM_SWREQ_UART_COMMUNICATION
*/
static int32_t UART_DrvGetTxStatus(UART_HwIDType aId, UART_TxStatusType *aStatus)
{
    int32_t ret = BCM_ERR_INVAL_PARAMS;
    if (aId < UART_MAX_HW_ID) {
        *aStatus = 0UL;
