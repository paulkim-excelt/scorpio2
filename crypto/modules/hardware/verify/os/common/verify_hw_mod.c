...
#define BRCM_SWDSGN_CRYPTO_VERIFY_HW_ECDSA384_SUBMIT_PROC       (0xA114U)   /**< @brief #CRYPTO_VerifyHWECDSA384Submit      */
#define BRCM_SWDSGN_CRYPTO_VERIFY_HW_SIGVERIF_SUBMIT_PROC       (0xA115U)   /**< @brief #CRYPTO_VerifyHWSigVerifSubmit      */
#define BRCM_SWDSGN_CRYPTO_VERIFY_HW_SIGVERIF_DONE_PROC         (0xA116U)   /**< @brief #CRYPTO_VerifyHWSigVerifDone        */
#define BRCM_SWDSGN_CRYPTO_VERIFY_HW_PROCESS_UPDATE_MSG_PROC    (0xA117U)   /**< @brief #CRYPTO_VerifyHWProcessUpdateMsg    */
#define BRCM_SWDSGN_CRYPTO_VERIFY_HW_PROCESS_MSG_PROC           (0xA118U)   /**< @brief #CRYPTO_VerifyHWProcessMsg          */
#define BRCM_SWDSGN_CRYPTO_VERIFY_HW_EVENT_HANDLER_PROC         (0xA119U)   /**< @brief #CRYPTO_VerifyHWEventHandler        */
#define BRCM_SWDSGN_CRYPTO_VERIFY_HW_GET_EVENT_MASK_PROC        (0xA11AU)   /**< @brief #CRYPTO_VerifyHWGetEventMask        */
#define BRCM_SWDSGN_CRYPTO_VERIFY_HW_MODULE_GLOBAL              (0xA11BU)   /**< @brief #CRYPTO_VerifyModule                */
/** @} */
...
/**
    ...
    @trace #BRCM_SWREQ_CRYPTO_VERIFY_HARDWARE
*/
#define CRYPTO_VERIFY_HW_EVENT_MASK (MODULE_EVENT | PKAEvent | SYMMCRYPTEvent)
...
typedef struct sCRYPTO_VerifyHWAlgoCtxType {
    ...
    SYMMCRYPT_ContextHandleType digestCtxHdl;                   /**< @brief Symmcrypt driver context handle         */
...
static int32_t CRYPTO_VerifyHWInitBigUintLimbs(const uint32_t aNumLimbs, const uint8_t *const aNumBigEndian, uint32_t *const aLimbs)
{
    ...
    if (aNumLimbs <= tmpBufLenWords) {
        ...
        memcpy(&tmpBuf, aNumBigEndian, aNumLimbs * 4UL);
...
static int32_t CRYPTO_VerifyHWResetStateHandler(BCM_StateType aRequestedState)
{
    ...
    if (BCM_ERR_OK == ret) {
        mvk->keyAlgo = keyType;
        if (CRYPTO_ALGO_RSA2K == keyType) {
            mvk->keyPaddingAlgo = CRYPTO_ALGO_PKCS21;
            mvk->keyLenWords = 64UL;
            mucSecMode = MCU_SECURITY_MODE_RSA;
        } else if (CRYPTO_ALGO_ECDSA384 == keyType) {
            mvk->keyPaddingAlgo = 0UL;
            mvk->keyLenWords = 24UL;
            mucSecMode = MCU_SECURITY_MODE_ECC;
        } else {
            /* Auth Failure */
            mvk->keyLenWords = 0UL;
            mvk->keyAlgo = 0UL;
...
#endif
#if 1
    const uint8_t mvkECDSA384TestKey[96UL] = {
        0xa2,0x95,0x0c,0x5b,0xbb,0x4e,0x80,0x63,0x01,0x35,0xd6,0xb8,0x58,0x8d,0x96,0xa9,
        0xf9,0x6a,0x16,0x92,0xce,0x0f,0x4e,0x6e,0x5a,0x75,0xc2,0x06,0x4f,0xed,0x35,0x3c,
        0x16,0xe1,0x50,0xf4,0x88,0x12,0x39,0xa9,0xd2,0xc2,0x76,0xea,0x4c,0x08,0x9d,0x42,
        0x6b,0xa3,0x3d,0xa0,0x0f,0x99,0xce,0x57,0xc9,0xba,0x4e,0x49,0x35,0x5e,0x43,0xc6,
        0x2e,0xfb,0x55,0xf0,0x8f,0xcb,0x65,0xc8,0xb0,0x4e,0xeb,0x1c,0xd4,0x79,0x6d,0xcf,
        0x2e,0xfe,0x03,0x62,0x92,0x42,0x1f,0x7b,0x24,0xfb,0x17,0xb0,0x65,0x31,0xf3,0x0c,
    };
    memcpy(mvk->pubKey, mvkECDSA384TestKey, 96UL);
    mvk->keyLenWords = 24UL;
    mvk->keyAlgo = CRYPTO_ALGO_ECDSA384;
    mvk->keyPaddingAlgo = 0UL;
    keyType = CRYPTO_ALGO_ECDSA384;
#endif
    ...
    if (BCM_ERR_OK != ret) {
        ...
        MCU_SetSecurityMode(mucSecMode);
...
/**
    ...
    if (CRYPTO_VERIFY_HW_ALGO_CTX_STATE_UNINIT != algCtxState)
        ...
        if (CRYPTO_VERIFY_HW_ALGO_CTX_STATE_DIGEST_MORE_DATA == algCtxState)
            SYMMCRYPT_DestroyHashContext(aAlgCtx->digestCtxHdl)
    ...
    if (CRYPTO_VERIFY_HW_ALGO_CTX_STATE_UNINIT != algCtxState) {
        ...
        */
        ...
        if (CRYPTO_VERIFY_HW_ALGO_CTX_STATE_DIGEST_MORE_DATA == algCtxState) {
            ...
            const int32_t retVal = SYMMCRYPT_DestroyHashContext(aAlgCtx->digestCtxHdl);
            (void) retVal;
        }
...
/**
    ...
    if ((CRYPTO_ALGO_RSA2K == sigAlg) && (CRYPTO_ALGO_SHA256 == digestAlg) && (CRYPTO_ALGO_PKCS21 == paddingAlg))
        ...
        ret = BCM_ERR_OK
    else if ((CRYPTO_ALGO_ECDSA384 == sigAlg) && (CRYPTO_ALGO_SHA384 == digestAlg))
        expectedSigLen = 96UL
        messageLenBlockMask = 127UL
        ret = BCM_ERR_OK
    else
...
static int32_t CRYPTO_VerifyHWValidateInitParams(const CRYPTO_VerifyInitType *const aCryptInitParams)
{
    ...
    if ((CRYPTO_ALGO_RSA2K == sigAlg) && (CRYPTO_ALGO_SHA256 == digestAlg) && (CRYPTO_ALGO_PKCS21 == paddingAlg)) {
        ...
        /* Signatures are 96 bytes long and the message needs to be a multiple of 128 bytes */
        expectedSigLen = 96UL;
        messageLenBlockMask = 127UL;
        ret = BCM_ERR_OK;
    } else {
...
/**
    ...
    if (BCM_ERR_OK == ret)
        SYMMCRYPT_ContextHandleType ctxHdl = 0ULL
        ret = SYMMCRYPT_CreateHashContext(aCryptInitParams->hash, &ctxHdl)
...
static int32_t CRYPTO_VerifyHWProcessInitMsg(const CRYPTO_VerifyInitType *const aCryptInitParams, int32_t *const aVerifStatus, CRYPTO_VerifyHWAlgoCtxType *const aAlgCtx)
{
    ...
    ret = CRYPTO_VerifyHWValidateInitParams(aCryptInitParams);
    if (BCM_ERR_OK == ret) {
        /* Params OK. Try creating a SYMMCRYPT hash context */
        SYMMCRYPT_ContextHandleType ctxHdl = 0ULL;
        ret = SYMMCRYPT_CreateHashContext(aCryptInitParams->hash, &ctxHdl);
...
/**
    ...
    if ((aAlgCtx->consumedSize + aCryptUpdateParams->chunkSize) == aAlgCtx->expectedSize)
        ret = SYMMCRYPT_HashFinish(...)
    else
        ret = SYMMCRYPT_HashUpdate(...)
...
static int32_t CRYPTO_VerifyHWDigestSubmit(const CRYPTO_VerifyUpdateType *const aCryptUpdateParams, CRYPTO_VerifyHWAlgoCtxType *const aAlgCtx)
{
    ...
    if ((CRYPTO_VERIFY_HW_NUM_CTX <= CRYPTO_VerifyHWModCtx.digestCtxIdx) &&
        ...
        if ((aAlgCtx->consumedSize + aCryptUpdateParams->chunkSize) == aAlgCtx->expectedSize) {
            ret = SYMMCRYPT_HashFinish(
                aAlgCtx->digestCtxHdl,
                aCryptUpdateParams->chunkSize,
                aCryptUpdateParams->chunkData,
                (uint8_t *) aAlgCtx->digestLimbs
            );
        } else {
            ret = SYMMCRYPT_HashUpdate(
                aAlgCtx->digestCtxHdl,
        ...
        if (BCM_ERR_OK == ret) {
            const uint32_t ctxIdx = aAlgCtx - &CRYPTO_VerifyHWModCtx.algoCtx[0UL];
            aAlgCtx->consumedSize += aCryptUpdateParams->chunkSize;
            aAlgCtx->state = CRYPTO_VERIFY_HW_ALGO_CTX_STATE_DIGEST_IN_PROGRESS;
            CRYPTO_VerifyHWModCtx.digestCtxIdx = ctxIdx;
            CRYPTO_VerifyHWModCtx.lastProcessedCtxIdx = ctxIdx;
...
/**
    ...
    retVal = SYMMCRYPT_GetJobExecResult()
    ...
    CRYPTO_VerifyHWModCtx.digestCtx = NULL
    @endcode
*/
static void CRYPTO_VerifyHWDigestDone(void)
{
    ...
    const int32_t retVal = SYMMCRYPT_GetJobExecResult();
    if (BCM_ERR_BUSY == retVal) {
        /* Do nothing */
    } else if (BCM_ERR_OK != retVal) {
        /*
            ...
            SYMMCRYPT driver automatically invalidates the
        ...
        */
        ...
        /*
            ...
            If so, SYMMCRYPT driver would have invalidated the context
            for us.
        */
        if (algCtx->expectedSize == algCtx->consumedSize) {
            algCtx->digestCtxHdl = 0ULL;
...
/**
    @brief Submit ECDSA384 verification job to PKA

    @pre None

    @param[in]      aAlgCtx                     Context against which the ECDSA384 verification
                                                job is to be issued

    Return values are documented in reverse-chronological order
    @retval         #BCM_ERR_INVAL_STATE        Failed to populate the RSA args
    @retval         #BCM_ERR_OK                 RSA args populated
    ...
    retVal1 = CRYPTO_VerifyHWInitBigUintLimbs(24UL, aAlgCtx->signatureLimbs, aAlgCtx->signatureLimbs)
    retVal2 = CRYPTO_VerifyHWInitBigUintLimbs(12UL, aAlgCtx->digestLimbs, aAlgCtx->digestLimbs)

    if ((BCM_ERR_OK == retVal1) && (BCM_ERR_OK == retVal2))
        SECUTILS_ConstBigUintType sigR
        SECUTILS_ConstBigUintType sigS
        SECUTILS_ConstBigUintType pubKeyX
        SECUTILS_ConstBigUintType pubKeyY
        SECUTILS_ConstBigUintType digest

        sigR.numLimbs = 12UL;
        sigR.limbs = (aAlgCtx->signatureLimbs + 12UL);
        sigS.numLimbs = 12UL;
        sigS.limbs = aAlgCtx->signatureLimbs;
        digest.numLimbs = 12UL;
        digest.limbs = aAlgCtx->digestLimbs;
        pubKeyX.numLimbs = 12UL;
        pubKeyX.limbs = &CRYPTO_VerifyHWModMVK.pubKey + 12UL;
        pubKeyY.numLimbs = 12UL;
        pubKeyY.limbs = &CRYPTO_VerifyHWModMVK.pubKey;

        ret = PKA_EcdsaVerify(
            PKA_ECGROUP_SECP_384R1,
            &pubKeyX,
            &pubKeyY,
            &digest,
            &sigR,
    ...
    @endcode
*/
static int32_t CRYPTO_VerifyHWECDSA384Submit(CRYPTO_VerifyHWAlgoCtxType *const aAlgCtx)
{
    int32_t ret = BCM_ERR_UNKNOWN;

    /* Perform the necessary endianness conversions  */
    const int32_t retVal1 = CRYPTO_VerifyHWInitBigUintLimbs(24UL,
        (const uint8_t *)aAlgCtx->signatureLimbs, aAlgCtx->signatureLimbs);
    const int32_t retVal2 = CRYPTO_VerifyHWInitBigUintLimbs(12UL,
        (const uint8_t *)aAlgCtx->digestLimbs, aAlgCtx->digestLimbs);
    ...
    if ((BCM_ERR_OK == retVal1) && (BCM_ERR_OK == retVal2)) {
        ...
        pubKeyX.numLimbs = 12UL;
        pubKeyX.limbs = ((const uint32_t *) &CRYPTO_VerifyHWModMVK.pubKey) + 12UL;

        pubKeyY.numLimbs = 12UL;
        pubKeyY.limbs = (const uint32_t *) &CRYPTO_VerifyHWModMVK.pubKey;
...
/**
    ...
    else if (CRYPTO_ALGO_ECDSA384 == aAlgCtx->sigAlgo)
        ret = CRYPTO_VerifyHWECDSA384Submit(aAlgCtx)
    else
        ret = BCM_ERR_INVAL_STATE
...
static int32_t CRYPTO_VerifyHWProcessMsg(uint32_t aCtxIdx, uint32_t aMsgIdx)
{
    ...
    CRYPTO_VerifyType id = BCM_GET_CMDID(msg->id);
