...
int32_t MACSEC_SysCmdReq(MACSEC_CmdType aCmd, MACSEC_IOType *const aIO)
{
    ...
    if (aIO != NULL) {
        sysReqIO.sysReqID = SVC_MSC_ID;
        sysReqIO.magicID = SVC_MAGIC_MSC_ID;
        sysReqIO.cmd = aCmd;
...
#if 0
ISR2(MACSEC_IRQHandler)
{
    MACSEC_IRQEventType event;

    MACSEC_DrvIRQHandler(&event);
    BCM_SetEvent(MACSEC_IRQ_PROCESS_TASK, MACSEC_DRV_EVENT);
}
#endif
