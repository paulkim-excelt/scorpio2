...
*/
#define BRCM_SWDSGN_PKA_SYSCMDREQ_PROC                   (0xE101U)    /**< @brief #PKA_SysCmdReq             */
#define BRCM_SWDSGN_PKA_OSIL_NOTIFY_JOB_START_PROC       (0xE102U)    /**< @brief #PKA_OsilNotifyJobStart    */
#define BRCM_SWDSGN_PKA_OSIL_NOTIFY_JOB_COMPLETE_PROC    (0xE103U)    /**< @brief #PKA_OsilNotifyJobComplete */
#define BRCM_SWDSGN_PKA_IRQ_HANDLER_PROC                 (0xE104U)    /**< @brief #PKA_IRQHandler            */
/** @} */
...
int32_t PKA_SysCmdReq(PKA_CmdType aCmd, PKA_IOType *const aIO)
{
    ...
    if (NULL != aIO) {
        sysReqIO.sysReqID = SVC_PKA_ID;
        sysReqIO.magicID = SVC_MAGIC_PKA_ID;
        sysReqIO.cmd = aCmd;
...
{
    (void) aJobId;
    (void) SetEvent((TaskType)aTaskId, PKAEvent);
    return;
}

/** @brief PKA IRQ handler

    @trace #BRCM_SWARCH_PKA_RUN_JOB_SCHEDULER_PROC
