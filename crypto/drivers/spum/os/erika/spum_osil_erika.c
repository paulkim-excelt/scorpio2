...
*/
#define BRCM_SWDSGN_SPUM_SYSCMDREQ_PROC                   (0xE101U)    /**< @brief #SPUM_SysCmdReq             */
#define BRCM_SWDSGN_SPUM_OSIL_NOTIFY_JOB_START_PROC       (0xE102U)    /**< @brief #SPUM_OsilNotifyJobStart    */
#define BRCM_SWDSGN_SPUM_OSIL_NOTIFY_JOB_COMPLETE_PROC    (0xE103U)    /**< @brief #SPUM_OsilNotifyJobComplete */
#define BRCM_SWDSGN_SPUM_DMA_ERR_IRQ_HANDLER_PROC         (0xE104U)    /**< @brief #SPUM_DMACErrIRQHandler     */
#define BRCM_SWDSGN_SPUM_DMA_TC_IRQ_HANDLER_PROC          (0xE105U)    /**< @brief #SPUM_DMACTcIRQHandler      */
/** @} */
...
int32_t SPUM_SysCmdReq(SPUM_CmdType aCmd, SPUM_IOType *const aIO)
{
    ...
    if (NULL != aIO) {
        sysReqIO.sysReqID = SVC_SPU_ID;
        sysReqIO.magicID = SVC_MAGIC_SPU_ID;
        sysReqIO.cmd = aCmd;
...
{
    (void) aJobId;
    (void)SetEvent((TaskType)aTaskId, SPUMEvent);
    return;
}

/** @brief DMA error IRQ handler

    @trace #BRCM_SWARCH_SPUM_RUN_JOB_SCHEDULER_PROC
    @trace #BRCM_SWREQ_SPUM_KERNEL_HANDLER
*/
ISR2(SPUM_DMACErrIRQHandler)
{
    /* Call the scheduler */
...
/** @brief DMA TC IRQ handler

    @trace #BRCM_SWARCH_SPUM_RUN_JOB_SCHEDULER_PROC
    @trace #BRCM_SWREQ_SPUM_KERNEL_HANDLER
*/
ISR2(SPUM_DMACTcIRQHandler)
{
    /* Call the scheduler */
    SPUM_RunJobScheduler();
    return;
